...
function errorText(code) {
  return ({
    ...
    invalid_compressed_data:"PNG 压缩数据损坏，校验失败",
    crc_mismatch:"PNG 校验码错误，文件已损坏",
    truncated_chunk:"PNG 数据块被截断，文件已损坏",
    bad_image_data:"PNG 图像数据损坏，校验失败",
    bad_filter:"PNG 解码失败（异常滤波），文件可能已损坏",
    invalid_scale:"标尺数值非法（必须为有限正数，且在合理量程内）",
    invalid_magnification:"镜头倍率不支持",
    invalid_grid:"网格行列数非法",
    invalid_overlap:"重叠率必须在 5%–60% 之间",
    invalid_slice_id:"切片编号非法（仅允许字母、数字、空格、点、下划线、连字符）",
    slice_id_exists:"该切片编号已存在",
...
$("#mosaicCanvas").addEventListener("click", async evt => {
  ...
  // A double-click to close a polygon also dispatches the second click, which
  // would otherwise append the closing vertex twice. Ignore a point that lands
  // on the current last vertex (within a couple of device pixels).
  if (tool === "area" && pending.length) {
    const last = pending[pending.length - 1];
    if (Math.abs(last.x - p.x) <= 2 && Math.abs(last.y - p.y) <= 2) {
      drawPending($("#mosaicCanvas").getContext("2d"));
      return;
    }
  }
  pending.push(p);
...
$("#mosaicCanvas").addEventListener("dblclick", async evt => {
  ...
  // Defensive: the double-click repeats the last click position; drop a
  // trailing duplicate vertex so the polygon is never submitted with one.
  ...
  if (pts.length < 3) { flash("面积至少需要 3 个不同顶点。", "err"); pending = []; renderMosaic(); return; }
  try { await submitMeasurement({ type:"area", points:pts }); pending = []; }
  catch (e) { flash(errorText(e.message), "err"); pending = []; renderMosaic(); }
});
