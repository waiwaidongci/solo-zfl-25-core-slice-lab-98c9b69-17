...
export class Store {
  ...
  sliceDir(sliceId) {
    return join(this.imagesDir, sliceId);
  }

  tilePath(sliceId, key) {
    return join(this.sliceDir(sliceId), `tile-${key}.png`);
  }

  mosaicPath(sliceId) {
    return join(this.sliceDir(sliceId), "mosaic.png");
  }

  privateStagePath(sliceId, tag) {
    return join(this.sliceDir(sliceId), `.stage-${tag}-${process.pid}-${this.uploadSeq()}`);
  }
  ...
  tileBackupPath(sliceId, key) {
    return join(this.sliceDir(sliceId), `.stage-tile-backup-${key}-${process.pid}-${this.uploadSeq()}`);
  }
  ...
  async reconcile() {
    ...
    for (const sliceId of entries) {
      const dir = this.sliceDir(sliceId);
      const slice = this.findSlice(sliceId)?.slice;
      let files = [];
